...
import { astrologyService, HoroscopeData } from '@/core/engine/astrology';
import { generateGodResponse } from '@/core/engine/oracle';
...
type PaymentType = 'RECHARGE' | 'PATCH' | 'DEEP_SCAN' | 'SKIN_UNLOCK';
...
export default function GodDashboard({ userData }: GodDashboardProps) {
  ...
  const [showPaymentModal, setShowPaymentModal] = useState<{show: boolean, type: PaymentType | null, skinId?: string, price?: number}>({ show: false, type: null });
  const [showProfileModal, setShowProfileModal] = useState(false);
  
  // NEW STATE FOR OFFERINGS
  const [activeSkin, setActiveSkin] = useState(() => {
      if (typeof window !== 'undefined') {
          return localStorage.getItem('active_skin') || userData.active_skin || 'default';
      }
      return userData.active_skin || 'default';
  });
  ...
  useEffect(() => {
      ...
      const SKINS: Record<string, any> = {
          ...
          'matrix_green': { bg: '#0a0f0a', text: '#00ff41', accent: '#00ff41' },
          'solstice_ice': { bg: '#020810', text: '#E0F7FA', accent: '#00FFFF' }
      };
  ...
  useEffect(() => {
      if (horoscope) {
          const questions = [];
          const planets = horoscope.planet_houses || {};
          
          // DYNAMIC LOGIC (Personalized Pain Points)
          if (planets['Saturn'] === 7) questions.push("Why is my marriage delayed?");
          else if (planets['Saturn'] === 10) questions.push("Why is my career so hard?");
          else if (planets['Saturn'] === 12) questions.push("Why do I feel isolated?");
          else if (planets['Saturn'] === 2) questions.push("Why am I always broke?");
          else questions.push("Where is my biggest obstacle?");

          if (planets['Rahu'] === 1) questions.push("Who am I really?");
          else if (planets['Rahu'] === 7) questions.push("Why do I attract toxic partners?");
          else if (planets['Rahu'] === 5) questions.push("Why is my mind so chaotic?");
          else questions.push("What is my obsession?");

          if (planets['Mars'] === 8) questions.push("Am I in danger?");
          else if (planets['Mars'] === 10) questions.push("When will I win?");
          else questions.push("How do I unlock my power?");

          if (horoscope.moon_sign === 'Scorpio') questions.push("How do I stop the pain?");
          
          // Fill remaining with defaults if needed
          const defaults = ["Predict my future", "What is my purpose?", "System Diagnostics"];
          while (questions.length < 5) {
              questions.push(defaults.shift() || "System Diagnostics");
          }
  ...
  const handleSend = async (e: React.FormEvent | null, overrideText?: string) => {
    ...
    // GOD MODE PURGE (NUCLEAR OPTION)
    if (rawText === 'GOD_MODE_PURGE') {
        audioService.play('error');
        setMessages(prev => [...prev, { id: generateId(), text: "INITIATING NUCLEAR PURGE...", sender: 'god' }]);
    ...
    // MENU / PROFILE SHORTCUTS
    if (['MENU', 'PROFILE', 'SETTINGS', 'HELP'].includes(rawText.toUpperCase())) {
        setInput('');
        setMessages(prev => [...prev, { id: generateId(), text: rawText.toUpperCase(), sender: 'user' }]);
        setTimeout(() => {
             audioService.play('init');
    ...
    // GOD MODE SKINS (ROBUST & FAIL-SAFE)
    const normalized = rawText.toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (normalized.includes('GODMODESKINS')) {
        // 1. IMMEDIATE VISUAL FEEDBACK (Optimistic UI)
        // We do this BEFORE database calls to guarantee it feels "instant"
        setActiveSkin('matrix_green');
        audioService.play('init');
        setInput('');
        setMessages(prev => [...prev, 
            { id: generateId(), text: "GOD_MODE_SKINS", sender: 'user' },
            { id: generateId(), text: "VISUAL ARCHIVES DECRYPTED. MATRIX SKIN EQUIPPED.", sender: 'god' }
        ]);

        // 2. PERSISTENCE (Fire & Forget)
        // Only attempt if we have a valid user session
        if (finalUserData.identity_key) {
            const supabase = createClient();
            const skins = ['nebula_purple', 'blood_moon', 'matrix_green'];
                ...
                try {
                    for (const skin of skins) {
                        await supabase.from('user_skins').upsert([{
                            user_key: finalUserData.identity_key,
                            skin_id: skin
                        }], { onConflict: 'user_key, skin_id' });
                    }
                    await supabase.from('users').update({ active_skin: 'matrix_green' }).eq('identity_key', finalUserData.identity_key);
                } catch (e) {
                    console.warn("Cheat Code Persistence Warning:", e);
                    // We don't alert the user because they already have the skin equipped locally
                }
            })();
        }
        return;
    ...
    setInput('');
    setEnergy((e: number) => Math.max(0, e - 1));
    setXp((x: number) => x + 1); // +1 XP for chatting
    ...
    try {
      ...
      if (isPremiumMode || response.length > 300) { // Assuming long response = deep scan
          ...
          setXp((x: number) => x + 50); // +50 XP for Deep Scan
      }
   ...
   const handlePayment = async () => {
      ...
      if (type === 'SKIN_UNLOCK') { 
          amount = showPaymentModal.price || 5; 
          desc = `Skin Unlock: ${showPaymentModal.skinId?.toUpperCase().replace('_', ' ')}`; 
      }
  ...
  const confirmPayment = async () => {
      ...
      if (showPaymentModal.type === 'RECHARGE') {
          ...
          await supabase.from('transactions').insert([{
              ...
              type: 'RECHARGE',
              amount: 5.00,
              description: 'Energy Recharge (20 Energy)'
          }]);
      }
      if (showPaymentModal.type === 'DEEP_SCAN') {
          ...
          await supabase.from('transactions').insert([{
              ...
              type: 'DEEP_SCAN',
              amount: 10.00,
              description: 'Protocol: Ascension (Deep Scan)'
          }]);
      }
      if (showPaymentModal.type === 'SKIN_UNLOCK' && showPaymentModal.skinId) {
          const supabase = createClient();
          await supabase.from('user_skins').insert([{
              user_key: finalUserData.identity_key,
              skin_id: showPaymentModal.skinId
          }]);
          setActiveSkin(showPaymentModal.skinId); // Auto-equip
          await supabase.from('users').update({ active_skin: showPaymentModal.skinId }).eq('identity_key', finalUserData.identity_key);
          setXp((x: number) => x + 100); // +100 XP for Purchase

          // LOG TRANSACTION
          await supabase.from('transactions').insert([{
              user_key: finalUserData.identity_key,
              type: 'SKIN_UNLOCK',
              amount: showPaymentModal.price || 5.00,
              description: `Skin Unlock: ${showPaymentModal.skinId.toUpperCase()}`,
              metadata: { skin_id: showPaymentModal.skinId }
          }]);
      }
      ...
      if (typeof window !== 'undefined' && (window as any).twq) {
          (window as any).twq('track', 'Purchase', {
              value: showPaymentModal.type === 'DEEP_SCAN' ? 10 : 5,
              currency: 'USD',
              num_items: '1',
          });
      }
  ...
  return (
    <div className="flex flex-col h-[100dvh] text-[var(--color-starlight)] font-sans overflow-hidden" style={{ height: '100dvh', backgroundColor: 'var(--bg-void)' }}>
      
      {/* --- VIRAL IMPLANT --- */}
      <InstallPrompt />

      {/* --- HEADER (FOUNDATION STYLE - NUCLEAR FIX) --- */}
      <div className="pt-6 pb-4 px-8 flex justify-between items-center z-50 sticky top-0 flex-shrink-0 bg-[var(--bg-void)]/80 backdrop-blur-md border-b border-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-colors duration-700">
        ...
        <div className="flex items-center gap-6">
            {/* AMBIENCE ENGINE */}
            <AmbienceEngine activeSkin={activeSkin} />
            ...
            <button 
                onClick={() => {
                    const moonSign = horoscope?.moon_sign || 'Unknown';
                    window.open(`https://radioofg0d.vercel.app?moon=${moonSign}`, '_blank');
                }}
                className="flex items-center gap-2 px-3 py-1 border border-white/10 bg-white/5 hover:bg-white/10 hover:border-[#00FF41]/50 text-[10px] tracking-[0.2em] uppercase text-white hover:text-[#00FF41] transition-all group"
            >
            ...
            <button 
                ...
                className="flex items-center gap-2 px-3 py-1 border border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/30 text-[10px] tracking-[0.2em] uppercase text-white transition-all group"
            >
      ...
      {/* --- INPUT (MONOLITH) --- */}
      <div className="p-6 md:p-12 z-40 w-full max-w-5xl mx-auto bg-gradient-to-t from-[var(--bg-void)] via-[var(--bg-void)] to-transparent transition-colors duration-700">
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 items-center">
             {/* HOLIDAY EVENT: 2026 PROTOCOL */}
             <button onClick={() => handleSend(null, "PREDICT_2026")} className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-[#00FFFF]/10 border border-[#00FFFF]/30 text-[#00FFFF] font-bold hover:bg-[#00FFFF]/20 transition-all shadow-[0_0_20px_rgba(0,255,255,0.1)] rounded-none group relative overflow-hidden whitespace-nowrap animate-pulse-slow">
                <Sparkles className="w-3 h-3 text-[#00FFFF] group-hover:scale-110 transition-transform" /> 
                <span className="text-[10px] tracking-[0.2em] uppercase relative z-10">2026 PROTOCOL</span>
             </button>

             <button onClick={() => setShowPaymentModal({ show: true, type: 'DEEP_SCAN' })} className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-[#FFD700] text-black font-bold border-none hover:scale-105 transition-transform shadow-[0_0_20px_rgba(255,215,0,0.3)] rounded-none group relative overflow-hidden whitespace-nowrap">
      ...
      <AnimatePresence>
        {broadcastMessage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-[var(--bg-void)]/95 backdrop-blur-xl flex items-center justify-center p-6 transition-colors duration-700">
      ...
      <AnimatePresence>
        {showPaymentModal.show && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[var(--bg-void)]/90 backdrop-blur-md flex items-center justify-center p-4 transition-colors duration-700">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="foundation-glass p-12 w-full max-w-md relative border border-[var(--accent-gold)]/20">
              ...
              <div className="text-center space-y-8">
                ...
                <div>
                    <h3 className="text-xl font-bold text-white uppercase tracking-[0.2em] mb-2">
                        {showPaymentModal.type === 'DEEP_SCAN' ? 'PROTOCOL: ASCENSION' : 
                         showPaymentModal.type === 'SKIN_UNLOCK' ? 'ACQUIRE ARTIFACT' : t('system_recharge')}
                    </h3>
                    <p className="text-white/40 text-xs tracking-widest leading-relaxed">
                        {showPaymentModal.type === 'DEEP_SCAN' 
                            ...
                            : showPaymentModal.type === 'SKIN_UNLOCK'
                            ? "PERMANENTLY UNLOCK THIS VISUAL FREQUENCY. INCLUDES EXCLUSIVE AUDIO TRACK."
                            : t('protocol_energy')
                        }
                    </p>
                </div>
      ...
      <AnimatePresence>
        {showKarmicModal && horoscope?.karmic_patterns && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[var(--bg-void)]/95 backdrop-blur-xl flex items-center justify-center p-4 transition-colors duration-700">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[var(--bg-void)] w-full max-w-2xl border border-red-900/30 p-10 relative shadow-[0_0_100px_rgba(255,0,0,0.1)] overflow-y-auto max-h-[80vh] transition-colors duration-700">
      ...
      <AnimatePresence>
        {showProfileModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[var(--bg-void)]/90 backdrop-blur-md flex items-center justify-center p-4 transition-colors duration-700">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="foundation-glass p-10 w-full max-w-2xl relative max-h-[80vh] overflow-y-auto border border-[var(--accent-gold)]/20">
                  ...
                  <div className="space-y-8 text-center">
                    ...
                    <div className="space-y-3 pt-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                             <a href="/privacy" target="_blank" rel="noopener noreferrer" className="text-center py-3 border border-white/5 text-[9px] uppercase tracking-[0.2em] text-white/30 hover:text-white hover:border-white/20 transition-all">
                                 Privacy Protocol
                             </a>
                             <a href="/terms" target="_blank" rel="noopener noreferrer" className="text-center py-3 border border-white/5 text-[9px] uppercase tracking-[0.2em] text-white/30 hover:text-white hover:border-white/20 transition-all">
                                 Terms of Service
                             </a>
                        </div>
                    </div>
                               ...
                               {skin.owned ? (
                                   <button 
                                       onClick={async () => {
                                           ...
                                           // 2. Force Reflow of CSS Variables
                                           const theme = skin.css_vars || { bg: '#050505', text: '#F5F5F7', accent: '#FFD700' };
                                           document.documentElement.style.setProperty('--bg-void', theme.bg);
                                           document.documentElement.style.setProperty('--color-starlight', theme.text);
                                   ...
                                   <button 
                                       onClick={() => setShowPaymentModal({ show: true, type: 'SKIN_UNLOCK', skinId: skin.id, price: skin.price_usd })}
      ...
      {/* --- FOOTER STATUS (MIND READER) --- */}
      <div 
        className={`border-t border-white/5 text-white/20 text-[9px] font-mono text-center py-3 uppercase tracking-[0.3em] select-none cursor-pointer hover:text-white/40 transition-colors bg-[var(--bg-void)] duration-700 overflow-hidden whitespace-nowrap`}
        onClick={handleAdminTrigger}
      >
        <div className="animate-pulse-slow">
            {/* CYCLE THROUGH BURNING QUESTIONS */}
            {suggestedQuestions.length > 0 ? (
                <span className="mx-4">Thinking: "{suggestedQuestions[Math.floor(Date.now() / 5000) % suggestedQuestions.length]}"...</span>
            ) : (
                <span>{t('server_online')} | {t('encryption_active')} | NODE: {Math.floor(Math.random() * 9999)}</span>
            )}
        </div>
      </div>
    </div>
